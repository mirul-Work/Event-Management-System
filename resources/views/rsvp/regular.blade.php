...
<div class="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4">
    ...
    <div class="bg-white shadow-lg rounded-lg p-6 w-full max-w-lg">
        ...
        <!-- Acceptance Form -->
        <form action="{{ route('rsvp.submit', ['seat_category' => 'regular', 'token' => $attendee->token]) }}" method="POST" class="mb-4">
            @csrf
        ...
        <!-- Rejection Form -->
        <form action="{{ route('rsvp.submit', ['seat_category' => 'regular', 'token' => $attendee->token]) }}" method="POST">
            @csrf
